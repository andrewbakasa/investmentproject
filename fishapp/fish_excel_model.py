...
class FishExcelReport(ExcelReport):
    project_title = 'PERI-URBAN FISH FARMING OUTPUTS'
    track_inputs = { 
                    'timing':{
                        'base_period':{},
                    ...
                    'production_inventory':{
                        'header': {},
                        'fish_per_tank_per_year': {}, 
                        'fingerlings_survival_rate': {},
                        'thousand': {}, 
                        'fish_weight_at_selling': {},
                        'num_of_tanks_targeted_for_construction': {},
                        'cum_tanks_under_fish': {},
                        'cum_tanks_under_harvesting': {},
                        'production_quantity': {},
                        'closing_inventory': {},
                     ...
                     'investment_cost':{
                        'header': {},
                        'total_land_for_tanks': {}, #changed here???
                        'cost_of_land_per_sqm': {},
                        'cost_of_machinery_per_tank': {}, 
                        'cost_of_building_per_sqm': {},
                        'total_tanks': {},
                        'cost_of_unit_tank': {},
                        'senior_debt_dynamic_parameter': {},
                        'initial_tanks_employed': {},
                        'fish_density': {},
                        'tank_length': {},
                        'tank_width': {},
                        'tank_depth': {},
                        'total_land_required': {},
                        'cost_of_tanks_constructed': {},
                        'cost_of_land': {},
    ...
    _production_inventory = {
        'fish_per_tank_per_year': {'title': 'Fish per tank per year','value': 0, 'units': 'NUMBER'},
        'fingerlings_survival_rate': {'title': 'Fingelings Survival rate','value': 1, 'units': 'PERCENT'},#duplicate
        ...
        'total_tanks': {'title': 'Total Tanks','value':None , 'units': 'NUMBER'},
    }	
    ...
    def _add_tankDesignParameters_section(self, w_sheet, row_index,total_wsheet_cols):
        ...
        itemlist =['num_of_tanks','length','width','depth','sqm','volume_of_water',
                   'density_per_cubic_metre','total_fish_per_tank_per_cycle',
                   'num_of_months_per_cycle','fish_per_tank_per_year',]
        ...
        for item in itemlist:
            if item in self.sister_model.tank_design_parameters:
                ...
                self._write_row_title_and_value3(w_sheet, colHeader, colValue, colUnits, row_index, 
                        self.sister_model.tank_design_parameters[item]['title'], self.sister_model.tank_design_parameters[item]['value'], cell_display_val, 
                        self._get_number_formats(_unit), _unit=='BLANK')

                  

                self.track_inputs['tank_design_parameters'][item]['row'] = row_index
                self.track_inputs['tank_design_parameters'][item]['unit'] = _unit
                self.track_inputs['tank_design_parameters'][item]['col'] = 6#redundant
                self.track_inputs['tank_design_parameters'][item]['value'] = self.sister_model.tank_design_parameters[item]['value'] 
                
                row_index +=1
                #skipp empty spaces 
                if item in ['sqm','volume_of_water','density_per_cubic_metre','total_fish_per_tank_per_cycle',]:
                    row_index +=1
                   
        
        #fill this place by retrievint from trackinputs-----
        self._update_tank_linkedcells(w_sheet)
        #-----------------------------------------------------
    
        #----leave blank for now: update later in Outputs
        self.track_inputs['fish_business_options']['header']['row'] = row_index
        row_index +=1
        #-----------------------------------------------------------
        for item in self.sister_model.fish_business_options.keys():
            ...
            self.track_inputs['fish_business_options'][item]['description'] = self.sister_model.fish_business_options[item]['description']            
            row_index +=1
        
        #---------------hide text for business options------------------------------------
        option_total= len(self.sister_model.fish_business_options.keys())
            ...
            if 'model_selection' in  self.track_inputs['outputs']:
                ...
                for item in list_:
                    ...
                    if i ==len(list_)-1:
                        ...
                        formulae_part += 'IF(Outputs!' + model_sel_cell + '="'+ self.sister_model.fish_business_options[item]['heading'] +'",' + colHeader + str(input_row_index) + ','
                    i+=1
                #---------------------------------------------------------------
    ...
    def _get_input_memory_details(self, input_parameter, search_start=0):
        ...
        intemlist = ['timing','prices','flags','financing','depreciation',
                    ...
                    'tank_design_parameters', 'production_inventory','cost_real', 'investment_cost']
        flag_found = False
    ...
    def _write_input_sheet(self, wb):
        ...
        #==========Prices Section =========================
        row_index +=2
        row_index = self._add_prices_section(input_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        ...
        #======================================+++++++++++++++
         ...
         #==========Tank Design Parameter Section =========================
        row_index +=2
        row_index = self._add_tankDesignParameters_section(input_sheet, row_index,total_wsheet_cols)
        #======================================+++++++++++++++
    ...
    def _update_sensitivity(self, sens_sheet,modelx):
        ...
        #ddddd
        item_list =[
               {'title':"Fingerling Price per 1000", "track_input_item": "sens_fingerlings_price_per_1000_units",'base_val':self.sister_model.fingerlings_price_per_1000_units, 'number_format':"NUMBER"},
               {'title':"Base Price",                      "track_input_item": "sens_base_price",          'base_val':self.sister_model.base_price,             'number_format':"NUMBER"},
               {'title':"Change in Price [Fish]",                 "track_input_item": "sens_change_in_price",      'base_val':self.sister_model.change_in_price,       'number_format':"PERCENT"},
               ...
               {'title':"Initial Tanks Employed",               "track_input_item": "sens_initial_tanks_employed" , 'base_val':self.sister_model.initial_tanks_employed, 'number_format':"INTEGER"},
               ...
               {'title':"Total Fish per tank per cycle", "track_input_item":  "sens_total_fish_per_tank_per_cycle", 'base_val':self.sister_model.total_fish_per_tank_per_cycle, 'number_format':"INTEGER"},
               ...
               {'title':"Dressed Weight @ Selling",                "track_input_item": "sens_fish_weight_at_selling",               'base_val':self.sister_model.fish_weight_at_selling,               'number_format':"NUMBER"},
    ...
    def _populate_productionInventory_section(self, w_sheet, item ):              
        ...
        if item=='cum_tanks_under_harvesting':
            r_index, c_index, found_state= self._retrieve_cell_row_colm('production_inventory','cum_tanks_under_harvesting')
            if found_state:
                span_ =self._get_span()
                first_slice_point = get_column_letter(9) + str(r_index)# D07
                second_slice_point = get_column_letter(9 + int(span_)) + str(r_index) # D39
        ...
        if item=='production_quantity':
            #=A*B*C*D/E * F
            #= fish_per_tank_per_year * fingerlings_survival_rate * fish_weight_at_selling * cum_tanks_under_fish/ thousand * OP
            formalue_string =[
                {'value':'=', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': 'A', 'var_type': 'variable','header': 'production_inventory', 'para': 'fish_per_tank_per_year','cell_type': 'single'},
                {'value': '*', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': "B", 'var_type': 'variable', 'header': 'production_inventory', 'para': 'fingerlings_survival_rate','cell_type': 'single'},
                {'value':'*', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': "C", 'var_type': 'variable', 'header': 'production_inventory', 'para': 'fish_weight_at_selling','cell_type': 'single'},
                {'value': '*', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': 'D', 'var_type': 'variable', 'header': 'production_inventory', 'para': 'cum_tanks_under_harvesting','cell_type': 'cell_range'},
                {'value': '/', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': '(', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': 'E', 'var_type': 'variable', 'header': 'production_inventory', 'para': 'thousand','cell_type': 'single'},
                {'value': '*', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': '1000', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},# fiah in gramms divide by1000
                {'value': ')', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': '*', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
        ...
        if item=='total_tanks':
            r_index, c_index, found_state= self._retrieve_cell_row_colm('production_inventory','total_tanks')
            if found_state:
                span_ =self._get_span()
                
                r_index2, _, found_state2= self._retrieve_cell_row_colm('production_inventory','num_of_tanks_targeted_for_construction')
                if found_state2: 
    ...
    def _populate_costs_section(self, w_sheet, item ):
        ...
        if item=='cum_tanks':
            # 
            cum_tanks_row_index, _, found_state_tanks= self._retrieve_cell_row_colm('production_inventory','cum_tanks_under_fish')
           
            r_index, _, found_state= self._retrieve_cell_row_colm('cost_real','cum_tanks')
        ...
        if item=='num_of_workers':
            ...
            r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','num_of_workers_per_tank')
            num_of_workers_per_tank_cell = '$' + get_column_letter(c_index) + '$' + str(r_index ) if found_state else '0'
            
            targeted_tanks_row_index, _, found_state_tanks= self._retrieve_cell_row_colm('cost_real','cum_tanks')
    ...
    def _add_investmentCost_section(self, w_sheet, row_index,total_wsheet_cols):
        ...
        itemlist =['total_land_for_tanks','cost_of_land_per_sqm','cost_of_machinery_per_tank', 
                   'cost_of_building_per_sqm','total_tanks','cost_of_unit_tank', 
                   'senior_debt_dynamic_parameter','initial_tanks_employed','fish_density',
                   'tank_length','tank_width','tank_depth',
                   'total_land_required','cost_of_tanks_constructed','cost_of_land',
        ...
        for item in itemlist:
            if item in self.sister_model.investment_cost:
                ...
                row_index +=1
                #skipp lines
                if item in ['senior_debt_dynamic_parameter','tank_depth', 'total_land_required',]:
                    row_index +=1 
    ...
    def _populate_investment_parameters_header_rowsDefunct(self, w_sheet):
        ...
        if found_state:
            ...
            #print(f'1. {first_slice_point},2. {second_slice_point}')
            #----------->
            i=0
            for cellObj in w_sheet[first_slice_point:second_slice_point]:# along row axis
                for cell in cellObj:
    ...
    def _link_tank_design_parameters_to_model_selected(self, w_sheet):
        ...
        tank_para = ['num_of_tanks','length', 'width',  'depth']
        investment_cost_para = ['total_tanks','tank_length', 'tank_width', 'tank_depth']
        # take min to be safe

          # total_land_for_tanks
        ########self._set_linkedcell(w_sheet, 'tank_design_parameters','num_of_tanks', sqm_cell)
       

        list_len= min(len(tank_para), len(investment_cost_para))
        for iter in range(list_len):
            ...
            r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters' ,tank_para[iter])
            if found_state:
    ...
    def _update_investment_parameters(self, w_sheet):
        para_list= ['cost_of_land_per_sqm', 'cost_of_machinery_per_tank', 
            'cost_of_building_per_sqm', 'cost_of_unit_tank',
            'senior_debt_dynamic_parameter', 'initial_tanks_employed', 'fish_density', 
            'tank_length', 'tank_width', 'tank_depth',  'total_land_required',
        ]
        ...
        r_index, c_index, found_state= self._retrieve_cell_row_colm('investment_cost','total_tanks')
        if found_state:
            span_ =self._get_span()
            r_index2, _, found_state2= self._retrieve_cell_row_colm('production_inventory','num_of_tanks_targeted_for_construction')
            if found_state2: 
    ...
    def _add_prodInventory_section(self, w_sheet, row_index,total_wsheet_cols):
        ...
        itemlist =['fish_per_tank_per_year','fingerlings_survival_rate','thousand', 
                   'fish_weight_at_selling','num_of_tanks_targeted_for_construction','cum_tanks_under_fish', 
                   'cum_tanks_under_harvesting','production_quantity','closing_inventory',
                   'investment_roll_out_flag','total_tanks']
        #-----------------------------------
        for item in itemlist:
    ...
    def _populate_investmentcost_section(self, w_sheet, item ):
        ...
        if item=='cost_of_tanks_constructed':
            # 
            targeted_tanks_row_index, c_index, found_state_tanks= self._retrieve_cell_row_colm('production_inventory','num_of_tanks_targeted_for_construction')
            r_index, c_index, found_state= self._retrieve_cell_row_colm('investment_cost','cost_of_unit_tank')
            cost_of_unit_tank_cell = '$' + get_column_letter(c_index) + '$' + str(r_index ) if found_state else '0'
            # 
            
            r_index, c_index, found_state= self._retrieve_cell_row_colm('investment_cost','cost_of_tanks_constructed')
            if found_state:
                span_ =self._get_span()
                first_slice_point = get_column_letter(9) + str(r_index)# D07
                second_slice_point = get_column_letter(9 + int(span_)) + str(r_index) # D39
        ...
        if item=='cost_of_land':
            ...
            r_index, c_index, found_state= self._retrieve_cell_row_colm('investment_cost','total_land_for_tanks')
            total_land_for_tanks_cell = '$' + get_column_letter(c_index) + '$' + str(r_index ) if found_state else '0'
        ...
        if item=='cif_cost_of_machinery':
            r_index, c_index, found_state= self._retrieve_cell_row_colm('investment_cost','cost_of_machinery_per_tank')
            cost_of_machinery_per_tank_cell = '$' + get_column_letter(c_index) + '$' + str(r_index ) if found_state else '0'
           
            tanks_targeted_r_index, c_index, found_state_tanks_targeted= self._retrieve_cell_row_colm('production_inventory','num_of_tanks_targeted_for_construction')
    ...
    def _update_tank_linkedcells(self, w_sheet,):
        
        # fingerlings_survival_rate
        # r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','fingerlings_survival_rate')
        # fingerlings_survival_rate = get_column_letter(c_index) + str(r_index ) if found_state else None
        # num_of_months_per_cycle
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','num_of_months_per_cycle')
        num_of_months_per_cycle_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'

        
       
        # num_of_tanks
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','num_of_tanks')
        num_of_tanks_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        
        # length
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','length')
        length_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        
        # width
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','width')
        width_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'

        # depth
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','depth')
        depth_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        
        # volume_of_water #derived
        cal_val= '=' + length_cell + '*' + width_cell + '*' + depth_cell +'*1000'
        self._set_linkedcell(w_sheet, 'tank_design_parameters','volume_of_water', cal_val, "Calculated")

        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','volume_of_water')
        volume_of_water_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
      
        # total_fish_per_tank_per_cycle
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','total_fish_per_tank_per_cycle')
        total_fish_per_tank_per_cycle_cell = get_column_letter(c_index) + str(r_index ) if found_state else .00001
        
        # density per tank #derived
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','density_per_cubic_metre')
        density_per_cubic_metre = get_column_letter(c_index) + str(r_index ) if found_state else .00001
      
        cal_val= '=' + total_fish_per_tank_per_cycle_cell  + '*1000/' + volume_of_water_cell
        self._set_linkedcell(w_sheet, 'tank_design_parameters','density_per_cubic_metre', cal_val, "Calculated")

        # SQM Calculation
        cal_val = '=' + num_of_tanks_cell + '*' + length_cell + '*' + width_cell
        #print(f'formulae: {cal_val}')
        self._set_linkedcell(w_sheet, 'tank_design_parameters','sqm', cal_val, 'Calculated')
         # width
        
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','sqm')
        sqm_cell = None
        if found_state:
        ...
        # # PEN Calculation
        # cal_val = '=' + length_cell + '*' + width_cell + '*' + depth_cell
        # self._set_linkedcell(w_sheet, 'tank_design_parameters','volume_of_water',cal_val, 'Calculated')
        ...
        # fish_per_tank_per_year Calculation
        r_index, c_index, found_state= self._retrieve_cell_row_colm('tank_design_parameters','fish_per_tank_per_year')
        fish_per_tank_per_year = get_column_letter(c_index) + str(r_index ) if found_state else None
        
        cal_val= '=' + total_fish_per_tank_per_cycle_cell + '*' + str(number_of_months_in_a_year) +   '/' + num_of_months_per_cycle_cell
        self._set_linkedcell(w_sheet, 'tank_design_parameters','fish_per_tank_per_year', cal_val, "Calculated")

        # total_land_for_tanks
        self._set_linkedcell(w_sheet, 'investment_cost','total_land_for_tanks', sqm_cell)
       
       
        self._set_linkedcell(w_sheet, 'production_inventory','fish_per_tank_per_year', fish_per_tank_per_year)
        
        self._set_linkedcell(w_sheet, 'cost_real','fish_per_tank_per_year', fish_per_tank_per_year)
    ...
    def _add_sensitivity_section(self, wb, w_sheet, row_index,total_wsheet_cols):
        ...
        # Total tanks 	5 	#
        # Base price of fish in 2020 per ton 	4,500	LC
        # Total Fish in one pen per cycle 	20 	#
        ...
        itemlist =['domestic_inflation_rate','us_inflation_rate','change_in_price', 'accounts_receivable',
                   ...
                   'total_fish_per_tank_per_cycle' ,'fingerlings_survival_rate', 'fingerlings_price_per_1000_units',
                   'initial_tanks_employed']
        update_exclusion_list= []#[{'item_instance': 'investment_cost','item':'initial_tanks_employed'},]
        exclude_item= {}
        exclude_item['item_instance']='investment_cost'
        exclude_item['item']='initial_tanks_employed'
    ...
    def _populate_output_general(self,wksheet):
        ...
        array_dict= [
            {'target': 'land_under_tanks',                'source': 'total_land_for_tanks',             's_header': 'investment_cost',          's_wksheet': 'Inputs'},
            {'target': 'total_tanks',                    'source': 'total_tanks',           's_header': 'production_inventory',     's_wksheet': 'Inputs'},
            {'target': 'volume_of_water',                          'source': 'volume_of_water',                        's_header':  'tank_design_parameters','s_wksheet': 'Inputs'},
            {'target': 'total_fish_per_tank_per_cycle', 'source': 'total_fish_per_tank_per_cycle',  's_header': 'tank_design_parameters', 's_wksheet': 'Inputs'},
            {'target': 'fish_per_tank_per_year',           'source': 'fish_per_tank_per_year',         's_header': 'tank_design_parameters', 's_wksheet': 'Inputs'},
        
            {'target': 'base_price',                     'source': 'base_price',                       's_header':  'prices','s_wksheet': 'Inputs'},
            {'target': 'tonnes_produced_per_tank_per_year',   'source': 'tonnes_produced_per_tank_per_year',  's_header': 'cost_real',        's_wksheet': 'Inputs'},
            ]
        ...
        #3. total_fish_per_year
        #  = fish_per_tank_per_year  * total_tanks
        #  =A*B
        formalue_string =[
                {'value':'=', 'var_type': 'const', 'header': '', 'para': '',},
                {'value': 'A', 'var_type': 'variable','header': 'output_general', 'para': 'fish_per_tank_per_year'},
                {'value': '*', 'var_type': 'const', 'header': '', 'para': '',},
                {'value': "B", 'var_type': 'variable', 'header': 'output_general', 'para': 'total_tanks'},
            ]
        self._cell_formalue_fromstring(wksheet, formalue_string,'output_general', 'total_fish_per_year',None,'Output2')
        
        
        #3. total_fish_per_week
        #  =total_fish_per_year  /52
        #  =A*B
        formalue_string =[
                {'value':'=', 'var_type': 'const', 'header': '', 'para': '',},
                {'value': 'A', 'var_type': 'variable','header': 'output_general', 'para': 'total_fish_per_year'},
                {'value': '/52', 'var_type': 'const', 'header': '', 'para': '',},
            ]
        self._cell_formalue_fromstring(wksheet, formalue_string,'output_general', 'total_fish_per_week',None,'Output2')
         

        #3. annual_productivity_tons
        #  =tonnes_produced_per_tank_per_year  * total_tanks
        #  =A*B
        formalue_string =[
                {'value':'=', 'var_type': 'const', 'header': '', 'para': '',},
                {'value': 'A', 'var_type': 'variable','header': 'output_general', 'para': 'tonnes_produced_per_tank_per_year'},
                {'value': '*', 'var_type': 'const', 'header': '', 'para': '',},
                {'value': 'B', 'var_type': 'variable','header': 'output_general', 'para': 'total_tanks'},
    ...
    def _add_output_general(self, output_sheet, row_index):
        ...
        itemlist= [
                'land_under_tanks',  'total_tanks',    'volume_of_water', 'total_fish_per_tank_per_cycle','fish_per_tank_per_year',
                'total_fish_per_year',   'total_fish_per_week',   'base_price','tonnes_produced_per_tank_per_year', 'annual_productivity_tons',
                'annual_revenue_2yr', 'gross_profit_2yr',   'net_profit_2yr',  'gross_profit_margin_2yr',   
                'net_profit_margin_2yr' 
                ]
        ic_parameters = {
            'land_under_tanks': {'title': "Land Under Tanks",'value': None, 'units': 'SQM'},
            'total_tanks': {'title': "Total Tanks",'value': None, 'units': 'NUMBER'},
            'volume_of_water': {'title': "Volume Of Water/tank",'value': None, 'units': 'LITRES'},
            'total_fish_per_tank_per_cycle': {'title': "Total Fish in one tank per cycle",'value': None, 'units': 'NUMBER'},
            'fish_per_tank_per_year': {'title': "Total Fish per tank per year",'value': None, 'units': 'NUMBER'},
            'total_fish_per_year': {'title': "Total Fish per year",'value': None, 'units': 'NUMBER'},
            'total_fish_per_week': {'title': "Total Fish per week",'value': None, 'units': 'NUMBER'},

            'base_price': {'title': "Base price of fish per ton",'value': None, 'units': 'LC'},
            'tonnes_produced_per_tank_per_year': {'title': 'Fish Tonnes produced per tank per year','value': None, 'units': 'TONS'},
            'annual_productivity_tons': {'title': 'Annual Fish Productivity Tons','value': None, 'units': 'TONS'},
        ...
        for item in itemlist:
            ...
            if item in ic_parameters.keys():
                ...
                if item in ['annual_revenue_2yr','annual_productivity_tons', 
                        'total_fish_per_tank_per_cycle', 'total_fish_per_week'] :
                    row_index +=1
                    
        return row_index         
    def _update_sens_section(self, w_sheet, item ):
        #dynamically change initial tanks employed
        #eeeeeee
        number_format ='_(* #,##0.0_);_(* \(#,##0.0\);_(* "-"??_);_(@_)'
        number_format_integer ='_(* #,##0_);_(* \(#,##0\);_(* "-"??_);_(@_)'
        
        if item=='initial_tanks_employed':
            self._transfer_cell(w_sheet,'investment_cost','sens','initial_tanks_employed',number_format_integer, 'Inputs')
            
        if item=='senior_debt':
            self._transfer_cell(w_sheet,'investment_cost','sens','senior_debt_dynamic_parameter', '0.0%', 'Inputs','senior_debt')
        
        if item=='total_fish_per_tank_per_cycle':
            self._transfer_cell(w_sheet,'investment_cost','sens','fish_density',number_format_integer,'Inputs','total_fish_per_tank_per_cycle')
    ...
    def _add_costs_section(self, w_sheet, row_index,total_wsheet_cols):
        ...
        for item in self.sister_model.cost_real.keys():
            ...
            if item in self.sister_model.cost_real:
                ...
                #print(item,self.track_inputs['cost_real'][item])
                ...
                if item =='average_num_of_supervisors':
                   ...
                   row_index +=1 
                   self._add_sub_heading(w_sheet,'Cost of inputs per ton of Fish',row_index)

                elif item =='cost_of_electricity_per_tank_per_annum':
                   row_index +=1  
                   self._add_sub_heading(w_sheet,'Annual Price Change',row_index)

                #skip line after
                if item in ['fish_feed_price_per_kg', 'fish_weight_at_selling' , 
                            'total_input_costs_per_tank', 'tonnes_produced_per_tank_per_year']:
                   row_index +=1  
                # new line feed                
                row_index +=1
        #fill this place by retrievint from trackinputs-----
    ...
    def _update_cost_real_linkedcells(self, w_sheet,):
        ...
        #  fingerlings_price_per_1000_units derived cell
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','fingerlings_price_per_1000_units')
        fingerlings_price_per_1000_units_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'


        #  fish_per_tank_per_year derivd cell
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','fish_per_tank_per_year')
      
        fish_per_tank_per_year_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        #print(f'fish_per_tank_per_year : {r_index}, {c_index}, {found_state} {fish_per_tank_per_year_cell}')

      
        # fish_feed_price_per_kg
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','fish_feed_price_per_kg')
        fish_feed_price_per_kg_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        ...
        # fingerlings_survival_rate
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','fingerlings_survival_rate')
        fingerlings_survival_rate = get_column_letter(c_index) + str(r_index ) if found_state else None
        ...
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','fish_weight_at_selling')
        fish_weight_at_selling_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
     
        
        # Cost of fish per tank
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','cost_of_fish_per_tank')
        ...
        if found_state:
         ...
         # cost_of_fish_feed_per_tank

        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','commercial_feed_per_tank')
        commercial_feed_per_tank_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0' 

        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','cost_of_fish_feed_per_tank')
        ...
        # total_input_costs_per_tank
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','total_input_costs_per_tank')
        ...
        fish_per_tank_per_year_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        #print(f'2. fish_per_tank_per_year : {r_index}, {c_index}, {found_state} {fish_per_tank_per_year_cell}')

     
        # tonnes_produced_per_tank_per_year
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','tonnes_produced_per_tank_per_year')
        ...
        # cost_of_domestic_inputs_per_ton_of_fish_produced
        ...
        # fish_weight_at_selling linkedcell @ prosuxt
        self._set_linkedcell(w_sheet, 'production_inventory','fish_weight_at_selling', fish_weight_at_selling_cell) 
        ...
        fish_weight_at_selling_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
         ...
         # Commercial Feed Per Tank
        # redooo here...........
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','qnty_of_feed_per_fish_cycle')
        qnty_of_feed_per_fish_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        r_index, c_index, found_state= self._retrieve_cell_row_colm('cost_real','commercial_feed_per_tank')
        commercial_feed_per_tank_cell = get_column_letter(c_index) + str(r_index ) if found_state else '0'
        if found_state:
            c= get_column_letter(c_index)
    ...
    def _add_cal_purchases_nominal_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= ['annual_change_in_price_of_imported_inputs','cost_of_imported_inputs_per_ton_of_fish_produced',
                   ...
                   'annual_change_in_price_of_domestic_inputs', 'cost_of_domestic_inputs_per_ton_of_fish_produced', 
        ...
        ic_parameters = {
            ...
            'cost_of_imported_inputs_per_ton_of_fish_produced': {'title': 'Cost of imported inputs per ton of Fish produced','value': None, 'units': 'USD'},
            ...
            'cost_of_domestic_inputs_per_ton_of_fish_produced': {'title': 'Cost of domestic inputs per ton of Fish produced','value': None, 'units': 'LC'},
    ...
    def _populate_cal_purchases_nominal(self, w_sheet):
        ...
        self._transfer_cell(w_sheet,'cost_real','calc_purchases_nominal','cost_of_imported_inputs_per_ton_of_fish_produced',number_format, 'Inputs')
        ...
        # costs of inputs
        r_index, c_index, found_state= self._retrieve_cell_row_colm('calc_purchases_nominal','cost_of_imported_inputs_per_ton_of_fish_produced')
        ...
        self._transfer_cell(w_sheet,'cost_real','calc_purchases_nominal','cost_of_domestic_inputs_per_ton_of_fish_produced',number_format, 'Inputs')
        ...
        # costs of inputs
        r_index, c_index, found_state= self._retrieve_cell_row_colm('calc_purchases_nominal','cost_of_domestic_inputs_per_ton_of_fish_produced')
    ...
    def _write_calculation_sheet(self, wb):
        ...
        row_index +=2
        row_index = self._add_cal_inflation_price_indices_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        ...
        #==========Investment( Real) =========================
        row_index +=2
        row_index = self._add_cal_investment_cost_real_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Investment (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_investment_cost_nominal_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Production And Sales (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_production_sales_nominal_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Purchases (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_purchases_nominal_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Working Capital (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_working_capital_nominal_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++

        #==========Labour Cost(Nominal) =========================
        row_index +=2
        row_index = self._add_cal_labour_cost_nominal_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Calc Of Unit Production(Nominal) =========================
        row_index +=2
        row_index = self._add_cal_unit_of_production_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        

        #==========Finished Product Inventory Valuation (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_finished_product_inventory_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Loan Schedule (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_loan_schedule_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Residual (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_residual_values_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
        
        #==========Depreciation For Tax Purposes (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_depreciation_for_tax_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++

        #==========Income Tax Statement (Nominal) =========================
        row_index +=2
        row_index = self._add_cal_income_tax_statement_section(calculation_sheet, row_index,total_wsheet_cols,'Fish')
        #======================================+++++++++++++++
    ...
    def _add_cal_production_sales_nominal_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= ['cum_tanks_under_harvesting','production_quantity','closing_inventory',
        ...
        ic_parameters = {
            ...
            'domestic_price_index': {'title': 'Domestic Price Index','value': None, 'units': 'NUMBER'},
            'change_in_price': {'title': 'Change in real price of Fish','value': None, 'units': 'PERCENT'},
            'base_price': {'title': 'Base price of Fish per ton','value': None, 'units': 'LC'},
            'real_price': {'title': 'Real Price of Fish per ton','value': None, 'units': 'LC'},
            'nominal_price': {'title': 'Nominal Price of Fish per ton','value': None, 'units': 'LC'},
    ...
    def _add_cal_unit_of_production_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= [
            ...
            'direct_labour_cost_per_ton_of_fish_produced',
        ...
        ic_parameters = {
            ...
            'direct_labour_cost_per_ton_of_fish_produced': {'title': 'Direct Labour cost per ton of fish produced','value': None, 'units': 'LC'},
        ...
        for item in itemlist:
            ...
            if item in ic_parameters.keys():
                ...
                #go to newline
                row_index +=1 
                if item in ['direct_labour_cost_per_ton_of_fish_produced'] :
                    row_index +=1 
    ...
    def _populate_cal_unit_of_production(self, w_sheet):
        ...
        #3. Direct Labor cost per ton of fish produced
        ...
        self._sum_oflist_cell_range(w_sheet,list_,'cal_unit_of_production','direct_labour_cost_per_ton_of_fish_produced',const_list_)
        ...
        listA= [{'header': 'cal_unit_of_production', 'para': 'direct_labour_cost_per_ton_of_fish_produced', 'action': '+' }, 
    ...
    def _populate_cal_production_sales_nominal(self, w_sheet):
        ...
        self._transfer_cell_range(w_sheet,'production_inventory','cal_production_sales_nominal',
                    'cum_tanks_under_harvesting',number_format, 'Inputs') 
    ...
    def _add_cal_investment_cost_real_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        row_index +=1
        #==========Sub-heading=========================
        self._add_sub_heading(w_sheet,'Tanks(real)',row_index)
        row_index +=1

        itemlist= ['cost_of_tanks_constructed','investment_cost_of_tanks',
        ...
        ic_parameters = {
            ...
            'investment_cost_of_tanks': {'title': 'Investment cost of Tanks (real)','value': None, 'units': 'T_LC'},# linked cell
        ...
        for item in itemlist:
            ...
            if item in ic_parameters.keys():
                ...
                #go to newline
                row_index +=1 
                if item in ['investment_cost_of_tanks','investment_cost_of_land_real',
                            'total_cost_machinery','investment_cost_of_buildings_real','senior_debt'] :
                    row_index +=1 

                    if item =='investment_cost_of_tanks':
    ...
    def _add_cf_nominal_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= [
                ...
                'tanks_nominal','land_nominal','total_cost_machinery_nominal','buildings_nominal',
        ...
        ic_parameters = {
            ...
            #EXPENDITURES
            #--------     Investment
            'tanks_nominal': {'title': 'Tanks (nominal)','value': None, 'units': 'T_LC'},
    ...
    def _populate_cf_nominal(self, w_sheet):
        ...
        #8. investment_cost nominal : tanks, building, machinery & land
        source_string =[
            {'source': 'investment_cost_of_tanks',             'target': 'tanks_nominal',                'header':'calc_investment_cost_nominal'},
        ...
        source_string =[
            ...
            {'source': 'cost_of_electricity_per_tank_per_annum_nominal',       'target': 'cost_of_electricity_nominal',                         'header':'cal_labour_cost_nominal'},
        ...
        formalue_string =[
                {'header': 'cf_nominal', 'para': 'tanks_nominal'},
    ...
    def _add_cal_investment_cost_nominal_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= ['domestic_price_index','cost_of_tanks_constructed','investment_cost_of_tanks',
        ...
        ic_parameters = {
            ...
            # Tanks
            'cost_of_tanks_constructed': {'title': 'Cost of tanks constructed','value': None, 'units': 'T_LC'},
            'investment_cost_of_tanks': {'title': 'Investment cost of Tanks (nominal)','value': None, 'units': 'T_LC'},# linked cell
        ...
        for item in itemlist:
            ...
            if item in ic_parameters.keys():
                ...
                #go to newline
                row_index +=1 
                if item in ['domestic_price_index','investment_cost_of_tanks','investment_cost_of_land_nominal',
                            'total_cost_machinery_nominal','investment_cost_of_buildings_nominal','senior_debt'] :
                    row_index +=1 

                     #==========Sub-headings=========================
                    if item =='domestic_price_index':
                        self._add_sub_heading(w_sheet,'Tanks(nominal)',row_index)
                        row_index +=1
    ...
    def _populate_cal_residual_values(self, w_sheet):
        ...
        #==I42+I27
        #=A+B
        #=investment_cost_of_tanks +  total_cost_machinery
        formalue_string =[
                {'value':'=', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
                {'value': 'A', 'var_type': 'variable','header': 'calc_investment_cost_real', 'para': 'investment_cost_of_tanks','cell_type': 'cell_range'},
                {'value': '+', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
    ...
    def _populate_cal_investment_cost_nominal(self, w_sheet):
        ...
        self._transfer_cell_range(w_sheet,'calc_investment_cost_real','calc_investment_cost_nominal',
                    'cost_of_tanks_constructed',number_format,)        
        #------------Nominal cost of Tanks
        invest_tanks_r_index, _, found_state_invest_tanks= self._retrieve_cell_row_colm('calc_investment_cost_nominal','cost_of_tanks_constructed')
        ...
        r_index, c_index, found_state= self._retrieve_cell_row_colm('calc_investment_cost_nominal','investment_cost_of_tanks')
        if found_state:
            span_ =self._get_span()
            first_slice_point = get_column_letter(9) + str(r_index)# D07
            second_slice_point = get_column_letter(9 + int(span_)) + str(r_index) # D39
    ...
    def _add_cf_real_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= [
            ...
            'tanks_real','land_real','machinery_real','buildings_real',
        ...
        ic_parameters = {
            ...
            #EXPENDITURES
            #--------     Investment
            'tanks_real': {'title': 'Tanks','value': None, 'units': 'T_LC'},
    ...
    def _populate_cf_real(self, w_sheet):
        ...
        for i in range(len(source)):
            # LOOP
            #=A/B
            #=tanks_nominal /domestic_price_index 
            formalue_string =[
                    {'value':'=', 'var_type': 'const', 'header': '', 'para': '','cell_type': ''},
        ...
        source= ['tanks_nominal','land_nominal','total_cost_machinery_nominal','buildings_nominal']
        target = ['tanks_real','land_real','machinery_real','buildings_real',]
        for i in range(len(source)):
            #7. tanks_real
            #=A/B
        ...
        formalue_string =[
                {'header': 'cf_real', 'para': 'tanks_real'},
    ...
    def _add_cal_labour_cost_nominal_section(self, w_sheet, row_index,total_wsheet_cols, commodity_title='Fish'):
        ...
        itemlist= ['num_of_workers','annual_increase_salaries_workers','monthly_wage_per_worker',
            ...
            'OP','cost_of_electricity_per_tank_per_annum',
            'cost_of_electricity_per_tank_per_annum_nominal',
        ...
        ic_parameters = {
            ...
            'OP': {'title': 'Operation period','value': None, 'units': 'FLAG'},
            #'domestic_price_index': {'title': 'Domestic Price Index','value': None, 'units': 'NUMBER'},
            'cost_of_electricity_per_tank_per_annum': {'title': 'Cost Of Electricity per tank per annum','value': None, 'units': 'LC'},
            'cost_of_electricity_per_tank_per_annum_nominal': {'title': 'Cost of electricity (nominal)','value': None, 'units': 'T_LC'},
    ...
    def _populate_cal_labour_cost_nominal(self, w_sheet):
        ...
        self._transfer_cell(w_sheet,'cost_real','cal_labour_cost_nominal','cost_of_electricity_per_tank_per_annum',number_format, 'Inputs')
        
        #17. cost_of_electricity nominal
        list_= [{'header': 'cal_labour_cost_nominal', 'para': 'OP', 'action': '+' }, 
                {'header': 'cal_labour_cost_nominal', 'para': 'domestic_price_index', 'action': '*' },
                {'header': 'cal_production_sales_nominal', 'para': 'cum_tanks_under_harvesting', 'action': '*' }]
        
        const_list_= [{'header': 'cal_labour_cost_nominal', 'para': 'cost_of_electricity_per_tank_per_annum','action': '*' }, 
                {'header': 'calc_investment_cost_real', 'para': 'million_to_thousand' ,'action': '/'}]   

        self._sum_oflist_cell_range(w_sheet,list_,'cal_labour_cost_nominal','cost_of_electricity_per_tank_per_annum_nominal',const_list_)
