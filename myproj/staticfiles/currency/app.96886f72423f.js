        ...
        if(i_layer.options && i_layer.options.pane === "markerPane") {
         ...
         // console.log(x)
          ...
          if (tag_source_username.length>0 || tag_target_username.length>0){
              ...
              //Endure match is emanating from the logged on user
              if ((props.matching_partner ==true) && (tag_source_username == user_name)){
                  // match here
                   /* 
                  ...
                  if (props.distance < 1000){
                      ...
                      if (props.distance < 100){
                        ...
                        statement ="You client is now  " + props.distance + 'metres away.'
                        speakfunc(statement)
                        console.log('Updating DB NearebyDistance: @meters', props.distance)
                        updateDistanceCloserAjax(props.uid, props.distance)
                      }else {
                        statement ="You are now close, the distance left is " + props.distance + 'metres to your target'
                        speakfunc(statement)
                      }
              ...
              }else if (tag_source_username == user_name){  
    ...
    fetch(dataurl
      ...
      .then(function(data) {
        ...
        for (let i = 0; i < data.features.length; i++) {
          ...
          //console.log('MP::' ,MP);
          if (data.features[i].properties.matching_partner){
