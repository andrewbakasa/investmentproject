...
function set_map(map){
  ...
  function resize_bounds_inviewport(){
        bounds = L.latLngBounds()
        console.log("bounds: ", L.latLngBounds())
        ...
        map.eachLayer(function(i_layer){ 
            ...
            if(i_layer.options && i_layer.options.pane === "markerPane") {
              ...
              let lat_lng = i_layer.options.lat_lng//[i_layer.options.lat_lng.lat,i_layer.options.lat_lng.lang]
...
function append_html(val,RemoveTag_TagMe,tag_color,tag_class){
        var prod_url = ''
        ...
        a_html =`<div id="${val.uid}" class="col-lg-4 annotation">                   
                    ...
                    <a class="ajax_tag_me"  href="#" data-url="/m/tag_currency_ajax/${val.owner_id}/" 
                          ...
                          title='${RemoveTag_TagMe}'><i class="${tag_class}" style="color:${tag_color}"></i>${val.username}
...
function make_all_hidden_layers_available_to_map(){
  if (!has_matching_partner){  
    ...
    // if not matching partner add all hiden layers
    // refreshing to start again
...
function update_layer(map,i_layer, props,newContent,newtooltipContent,counter_ranked){
    ...
    if (nonEmptyTaglist(props.tag_source_target)){
        ...
        newLatLng= i_layer.getLatLng()
        
        //someoone has targeted me and its at match
        //th current matching is emanating from the logged on user
        if ((props.already_matched ==true) 
                  && loggeduserIsInSourceList(props.tag_source_target, user_name)==true
                  && props.matching_partner ==true
                  ){
            // match here
            /* 
            ...
            */
            ...
            curr_tooltipContent = counter_ranked +  '. You are matched <small>' + props.offer_symbol + nFormatter(props.value,2) +" @" + nFormatterdist(props.distance,1)+ "</small>"
            newtooltipContent = counter_ranked +  '. Matched <small>' + props.offer_symbol + nFormatter(props.value,2) +" @" + nFormatter(props.rate_expected,2)+ "</small>"
            i_layer.setTooltipContent(newtooltipContent)
            
            addMarkerToMapIfNotAlreadyAvailable(i_layer)
                      
            map.eachLayer(function(x_layer){               
              if (x_layer._id == props.uid){
                flash_message(x_layer,newtooltipContent,curr_tooltipContent,matchIcon,newContent)
              }
            })
            //map.addLayer(i_layer);
        ...
        }else if (((props.already_matched ==true) 
            && loggeduserIsInSourceList(props.tag_source_target, user_name)==false
            && props.matching_partner ==true
            )){ 
              //Match is cause by other markers/user other than logged user
              console.log('Match Found. Hide:',props.username)  
              hide_layer_if_alreadymatched(i_layer)
        }else if (props.suitor.includes(user_name) && (props.matching_partner ==false)) {
          
            console.log('matching_partner?', props.matching_partner)
            // suitor
            i_layer.setIcon(mytrackerIcon);
            let newtooltipContent =counter_ranked + '. Suitor <small>' + props.offer_symbol + nFormatter(props.value,2) +" @" + nFormatter(props.rate_expected,2)+ "</small>"
            i_layer.setTooltipContent(newtooltipContent)  
        
          
            curr_tooltipContent =counter_ranked +  '. <small> Click to accept suitor @' + nFormatterdist(props.distance,1)+ "</small>"
           
            i_layer.setTooltipContent(curr_tooltipContent)
            i_layer.setPopupContent(newContent)
            AddToMap_if_not_matching_partner(map, i_layer)
            map.eachLayer(function(x_layer){               
              if (x_layer._id == props.uid){
                flash_message(x_layer,newtooltipContent,curr_tooltipContent,mytrackerIcon,newContent)
              }
            })
        ...
        }else if (loggeduserIsInSourceList(props.tag_source_target, user_name)==true){ 
            ...
            newtooltipContent = counter_ranked +  '. Tracking <small>' + props.offer_symbol + nFormatter(props.value,2) +" @" + nFormatter(props.rate_expected,2)+ "</small>"
          
            i_layer.setTooltipContent(newtooltipContent)
            curr_tooltipContent = counter_ranked +  '. <small> You are tracking @' + nFormatterdist(props.distance,1)+ "</small>"
            i_layer.setTooltipContent(curr_tooltipContent)
            i_layer.setPopupContent(newContent)
            AddToMap_if_not_matching_partner(map, i_layer)
            map.eachLayer(function(x_layer){               
              if (x_layer._id == props.uid){
                flash_message(x_layer,newtooltipContent,curr_tooltipContent,taggedIcon,newContent)
             }
            })
            ...
            //console.log("Marker is Free:", props.username)
            let newtooltipContent = counter_ranked +  '. <small>' + props.offer_symbol + nFormatter(props.value,2) +" @" + nFormatter(props.rate_expected,2)+ "</small>"
            i_layer.setTooltipContent(newtooltipContent)  
            // remove layer if there is matching
            i_layer.setPopupContent(newContent)
            AddToMap_if_not_matching_partner(map, i_layer)
          ...
          /* 
          Possible answers
          1.matching_partner =true; 
          2. already_matched=true, 
          3. logged user {not=} [suitor, tag_source_target]
      ...
      // ********************marker is available**************************
        if (props.suitor.includes(user_name) && (props.matching_partner ==false)) {
...
function remove_layer_if_matching_partner(map,i_layer ){
  ...
  if (has_matching_partner){ 
    ...
    if (id_map.includes(id)){
      ...
      id_map= removeItemOnce(id_map,id)
    }
    layer_not_avail_compansate+=1 
   
  }
}
function AddToMap_if_not_matching_partner(map,i_layer ){
  ...
  if (has_matching_partner){ 
    ...
    //map remove 
    id= i_layer._id 
    //console.log('1111:>>>>',i_layer.feature.properties.username)
    map.removeLayer(i_layer)
    //console.log('IN MATCHING:::Removing from MAP:>>>>',i_layer.feature.properties.username)
   
    if (id_map.includes(id)){
      //console.log('IN MATCHING:::REMOVING',i_layer.feature.properties.username)
      id_map= removeItemOnce(id_map,id)
    } 
...
function addMarkerToMapIfNotAlreadyAvailable(i_layer){
    id= i_layer._id 
    if (!id_map.includes(id)){
      id_map.push(id)
      ...
      console.log('Marker already In MAP:',i_layer.feature.properties.username )
    }
}
...
function remove_layer_if_alreadymatched(map,i_layer , props, text){
  
    layers_hidden.push(i_layer)
    console.log('There is a Match, so HIDE : ', i_layer.feature.properties.username)
    map.removeLayer(i_layer)
    id= i_layer._id 
    if (id_map.includes(id)){
...
function hide_layer_if_alreadymatched(i_layer ){ 
    layers_hidden.push(i_layer)
    layer_not_avail_compansate+=1  
}
...
function flash_message(i_layer, permanenttooltipContent, temp_tooltipContent, Icon_CONST, newContent){
    i_layer.setTooltipContent(temp_tooltipContent)
    i_layer.setIcon(Icon_CONST);
    i_layer.setPopupContent(newContent)
   
    //object
    let id_header = {
      "id": i_layer._id,
      "header": permanenttooltipContent,
     }
    //push object
    ids_headers.push(id_header);
    ...
    timeout = setTimeout( ()=> {  
        ...
        map.eachLayer(function(p_layer){    
          let index= ids_headers.findIndex(x => x.id === p_layer._id); 
         // console.log(index, !(parseInt(index) == -1))       
          if (!(index == -1)){//if found match
            p_layer.setTooltipContent(ids_headers[index].header)
          }
        })
  ...
  if (nonEmptyTaglist(props.tag_source_target)){
      ...
      if ((props.already_matched ==true) 
            ...
            && props.matching_partner ==true
              ...
              //user match
              return ['Withdral Engagement' ,'#FFD700', 'fas fa-tag']   

      }else if (props.suitor.includes(user_name) && (props.matching_partner ==false)) {
        // no loggin user tag
        return ['Accept Suitor', '#00FF00', 'fas fa-tag fa-spin']
      
      }else if (loggeduserIsInSourceList(props.tag_source_target, user_name)==true){ 
        //tracking to no tracking
        return ['Remove Tracking' ,'#FF00FF', 'fas fa-tag']
      }else if (props.already_matched ==false) {
       //virgin: no one following 
        return ['Track Client' ,'#2b92eb', 'fas fa-tag fa-spin']
      }else {
        // matched one sided------- no reversed partner
        // no loggin user tag
        return ['Track Client' ,'#2b92eb', 'fas fa-tag fa-spin']
      }
  }else {
...
function myScript(){
  ...
  map.eachLayer(function(i_layer){      
    if(i_layer.options && i_layer.options.pane === "markerPane") {
          ...
          if (!id_map.includes(id)){
            id_map.push(id)
            //  if (i_layer.feature) {
            //     //console.log('ADDED to id_map:', i_layer.feature.properties.username)
            //  }else {
            //     //console.log('ADDED to id_map:', i_layer._id)
            //  }
          }
    }
  }) 
  ...
  if (search_val.length>0){            
    ...
    $('#total_projects_searched').html('Processing . . .')
  }
  // refresh bounds
  bounds = L.latLngBounds()
   // 'new bounds'
  ...
  // Download GeoJSON data with Ajax
    fetch(dataurl
      ...
      .then(function(data) {
        ...
        for (let i = 0; i < data.features.length; i++) {
          ...
          if (data.features[i].properties.already_matched 
               && loggeduserIsInSourceList(data.features[i].properties.tag_source_target, user_name)==true
               && data.features[i].properties.matching_partner ==true
               ){
            has_matching_partner=true;
            console.log("Matching Partner:>>", data.features[i].properties.username)
          }

        }

        if (has_matching_partner ==false) {
        ...
        L.geoJson(data, {
          ...
          onEachFeature: function onEachFeature(feature, layer) {
            ...
            TagOutput =get_Tag_title(props)
            RemoveTag_TagMe=TagOutput[0],tag_color=TagOutput[1], tag_class=TagOutput[2]
            a_html=append_html(props,RemoveTag_TagMe,tag_color,tag_class)                 
           $(a_html).bind("mouseover",function(){                    
            ...
            anchor_tag =`<a class="ajax_tag_me"  href="#" data-url="/m/tag_currency_ajax/${props.owner_id}/" 
                data-target="#modal-edit-product-div" style="color:inherit" 
                data-toggle="tooltip" data-placement="bottom" 
                title='${RemoveTag_TagMe}'><i class="${tag_class}" style="color:${tag_color}"></i>${props.username}
            </a> `
            ...
            newtooltipContent = '<small>' + counter_ranked+ '. '+ props.offer_symbol + nFormatter(props.value,2) +" @" + nFormatter(props.rate_expected,2)+ "</small>"
      ...
      //console.log('IDMAP', id_map)
      remove_orphaned_markers()


      get_bounds_and_resize_MapView()
      val = parseInt(id_map.length)- parseInt(1)
      output_ = "<b style='background-color:yellow'>" + search_str + "</b> [" + val +"]"
      $('#total_projects_searched').html("Results of " + output_ )
...
function remove_orphaned_markers(){
  map.eachLayer(function(i_layer){ 
    ...
    if(i_layer.options && i_layer.options.pane === "markerPane") {
          ...
          if (!id_map.includes(id)){
              map.removeLayer(i_layer)
              if (i_layer.feature) {
                console.log('REMOVED ORPHAN:',i_layer.feature.properties.username)
              }else {
                console.log('REMOVED ORPHAN:',i_layer._id)
              }
