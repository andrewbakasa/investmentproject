    ...
    if (nonEmptyTaglist(props.tag_source_target)){
        ...
        if ((props.already_matched ==true) 
            ...
            */
            ...
            //remove_layer_if_alreadymatched(map, i_layer, props, 'Removing' + props.username + ' in A-ELSE-IF-STATEMENT')
            //layer_not_avail_compansate += 1
        }else {
          // default....
          /* 
      ...
      // ********************marker is available**************************
        if (props.suitor.includes(user_name) && (props.matching_partner ==false)) {
          ...
          // remove layer if there is matching
          AddToMap_if_not_matching_partner(map, i_layer)
          //remove_layer_if_alreadymatched(map, i_layer, props, 'Removing' + props.username + ' in B-ELSE-IF-STATEMENT')
          ...
          */
            ...
            // remove layer if there is matching
            AddToMap_if_not_matching_partner(map, i_layer)
            //remove_layer_if_alreadymatched(map, i_layer, props, 'Removing' + props.username + ' in B-ELSE-STATEMENT')
        }
...
function AddToMap_if_not_matching_partner(map,i_layer ){
  ...
  if (has_matching_partner){ 
    ...
    map.removeLayer(i_layer)
    console.log('Removing from MAP:>>>>',i_layer.feature.properties.username)
   
    if (id_map.includes(id)){
      console.log('Id also removed',i_layer.feature.properties.username)
      id_map= removeItemOnce(id_map,id)
    } 
