     ...
     .then(function(data) {
       ...
       L.geoJson(data, {
         ...
         onEachFeature: function onEachFeature(feature, layer) {
           ...
           //-----------------------------
           anchor_tag =`<a data-toggle="tooltip" data-placement="bottom" title='View Currency Details' href="#">${props.username}</a>`
        
           if (props.image) {
             content = `<img  style="width:100%;height:100px; object-fit: cover;" src="${props.image}"/></br><p><i style='color:blue'>Summary</i>: ${props.description}</br>
             ...
             content = `<img                                                       src=""             /></br><p><i style='color:blue'>Summary</i>: ${props.description}</br>
                          <i style='color:blue'>Uname</i>: ${anchor_tag} &nbsp;<i class="fas fa-map-marker-alt"></i>${nFormatterdist(props.distance,1)} &nbsp;  ${props.offer_symbol}${nFormatter(props.value,2)} &nbsp;@${props.rate_expected}</p>`;
          ...
          layer.bindTooltip(
            '<small>' + props.offer_symbol + nFormatter(props.value,2) +"</small>",
              {
                direction: 'right',
                permanent: true,
                sticky: true,
                    ...
                    <h5>
                      <a data-toggle="tooltip" data-placement="bottom" 
                          title='View Currency Details' 
                           href="/e/p/d/${val.uid}/">${val.username}
        ...
        L.geoJson(data, {
          ...
          onEachFeature: function onEachFeature(feature, layer) {
           ...
           $(a_html).bind("mouseover",function(){                    
            ...
            layer.bindPopup(content)
            newtooltipContent = '<small>' + props.offer_symbol + nFormatter(props.value,2) +"</small>",
            layer.bindTooltip(
                newtooltipContent,
                {
                  direction: 'right',
