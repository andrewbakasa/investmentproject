/**
 * user_interface.js - Full Production Refactor
 * Implements: Anti-racing, BOQRow Event Isolation, and Activity UI Templates.
 */

$(document).ready(function() {
    let csrftoken = getToken('csrftoken');
    let isSubmitting = false; // The "Racing" Lock
    ...
    function checkReadyModel() {
        if ($('#run_simu').css("display") === 'block') return;
        $.ajax({
            type: 'POST',
            url: typeof check_url_ajax !== 'undefined' ? check_url_ajax : '/check_model_ready/',
            ...
            success: function(data) {
                if (data.data) {
                    ...
                    $('#progress_iter').text(data.status).show();
                }
            }
        });
    }
    ...
    /**
     * CENTRAL AJAX ENGINE (Prevents Racing)
     * Replaces the 1000+ lines of repetitive AJAX calls
     */
    function handleGenericSubmit(e, formElement, updateTableFunc, addTableFunc) {
        e.preventDefault();
        if (isSubmitting) return; // Block multiple clicks

        const $form = $(formElement);
        const $submitBtn = $form.find('button[type="submit"]');
        const isEdit = $form.hasClass('edit-mode') || $form.attr('class').includes('-edit');
        ...
        $.ajax({
            ...
            dataType: 'json',
            success: function(data) {
                if (data && (data.error === false || data.model || data.data)) {
                    if (isEdit && typeof updateTableFunc === 'function') {
                        updateTableFunc(data);
                    } else if (typeof addTableFunc === 'function') {
                        addTableFunc(data);
                    } else {
                        // Default fallback: Refresh to show changes
                        location.reload();
                    }
            ...
            error: (err) => console.error("Critical Submission Error:", err),
            complete: () => {
                isSubmitting = false;
    ...
    // BOQRow & Modal Fixes (27 March)
    $('body').on("click", ".close-modal", function(e) {
        e.stopPropagation(); // Stop row expansion
        $('#modal-usermodel').modal('hide');
    });

    // Activity UI Template - Load Form
    $('body').on('click', '.show-form-update', function(e) {
        e.stopPropagation();
        const btn = $(this);
        // Highlight current row with Indigo
        btn.closest('tr').addClass('bg-indigo-50/50').siblings().removeClass('bg-indigo-50/50');

        $.ajax({
            url: btn.attr("data-url"),
            type: 'GET',
            dataType: 'json',
            beforeSend: () => $('#modal-usermodel').modal('show'),
            success: (data) => $('#modal-usermodel .modal-content').html(data.html_form)
        });
    });

    // --- DOWNLOAD & PROGRESS LOGIC ---

    $('body').on('click', '.show-form-download', function(e) {
        e.stopPropagation();
        const btn = $(this);
        $('#run_simu').hide();
        $('.progressBar').fadeIn();
        ...
        $.ajax({
            ...
            success: function(data, status, xhr) {
                ...
                link.download = $('#user_doc_name').val() || 'Model_Export.xlsx';
                link.click();
    ...
    function updateProgress() {
        $.post('/check_count_ajax', { csrfmiddlewaretoken: csrftoken }, function(res) {
            const p = res.data.progress;
            $('.progressBar .bar').css('width', p + '%');
            $('.progressBar h3').text(p >= 100 ? "Generating File..." : p + " %");
        });
    }

    // --- FORM BINDINGS (Mapping all 1400 lines of intent) ---

    const mappings = [
        { selector: ".model-prices", update: updatePricesTable, add: addPricesTable },
        { selector: ".model-timing-assumptions", update: updateGeneric, add: addGeneric },
        { selector: ".model-financing", update: updateGeneric, add: addGeneric },
        { selector: ".model-taxes", update: updateGeneric, add: addGeneric },
        { selector: ".model-depreciation", update: updateGeneric, add: addGeneric },
        { selector: ".model-working-capital", update: updateGeneric, add: addGeneric },
        { selector: ".model-macroeconomic-parameters", update: updateMacroTable, add: null }
    ];

    mappings.forEach(m => {
        $('body').on("submit", `${m.selector}-edit, ${m.selector}-add`, function(e) {
            handleGenericSubmit(e, this, m.update, m.add);
        });
    });

    // --- TABLE UPDATERS (The "Logic" Core) ---

    function updatePricesTable(data) {
        const item = data.data || data.model;
        $("#pricesTable #prices-title .pricesData").html(item.title);
        $("#pricesTable #prices-base-price .pricesData").text('$' + numberWithCommas(item.base_price));
    }

    function addPricesTable(data) {
        // Redraws the price section after creation
        location.reload(); 
    }

    function updateMacroTable(data) {
        const source = data.data || data.model;
        const fields = [
            'local_inflation_rate', 'us_inflation_rate', 'exchange_rate', 
            'dividend_payout_ratio', 'num_of_shares', 'investment_costs_over_run_factor'
        ];
        
        fields.forEach(f => {
            $(`#macroeconomicparametersTable #macroeconomicparameters-${f} .macroeconomicparametersData`).text(source[f]);
        });
    }

    function updateGeneric(data) {
        const source = data.data || data.model;
        // Automatically finds any span/td with the name of the field and updates it
        Object.keys(source).forEach(key => {
            $(`.table-data[name="${key}"], .pricesData[name="${key}"]`).html(source[key]);
        });
    }
