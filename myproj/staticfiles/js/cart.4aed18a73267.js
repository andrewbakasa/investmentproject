...
$('body').on("click",".empty-cart",function(e){
    e.preventDefault();
    var url = '/e/empty_cart_ajax/'
  
    if(user === 'AnonymousUser'){
        // console.log('before:cart', typeof(cart) )
        //console.log(instanceof(cart))
        // console.log(Object.getOwnPropertyNames(cart))
        prop= Object.getOwnPropertyNames(cart)
        // console.log(Object.getOwnPropertyNames(cart).filter(function (p) {
        //     return typeof cart[p] === 'function';
        // }));

        for (let i = 0; i < prop.length; i++) {
            console.log('what there',cart[prop[i]])
            delete cart[prop[i]]
        }
    ...
    $.ajax({
        ...
        success: function(response){
            if(response.data){
                ...
                $('.close-modal').click()
                if (response.data) {
                    $("#o_total_qnty").html(`<h5>Items: <strong>${numberWithCommas(0)}</strong></h5>`)
                    $("#o_total_price").html(`<h5>Total: <strong>$${numberWithCommas(0)}</strong></h5>`)
                    $("#cart-total").text(0)
                    $("#span-cart-total").text(0)
                    $("#t-tbody").children().remove()
                    flip_cart_display()
                }
