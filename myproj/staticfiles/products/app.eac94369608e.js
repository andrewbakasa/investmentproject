    ...
    var layers_count =0
      ...
      if (bounds.isValid() && layers_count > 1) {
        ...
        if (var_fly_bounds == true){
          ...
          map.flyTo([parseFloat(user_loc_x), parseFloat(user_loc_y),],map.getZoom())
        }

         
      } else {
        //console.log('2. No valid bounds')
        // Invalid, fit world
        map.fitWorld();
      }
...
function getPosition(position){
  ...
  if (get_transition_metre(5)){// at every 5 metres change redrw map
    ...
    if (t1 !== null){
        ...
        if (dif_seconds > 15)// only go here id 30 seconds has passed
           //run promise
            myScript()
            //var_fly_bounds=false//
            //update time        
