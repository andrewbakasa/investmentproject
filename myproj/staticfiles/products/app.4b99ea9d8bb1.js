     ...
     .then(function(data) {
       ...
       L.geoJson(data, {
         ...
         onEachFeature: function onEachFeature(feature, layer) {
           ...
           //-----------------------------
           if (props.image) {
             content = `<img  style="width:100%;height:100px; object-fit: cover;" src="${props.image}"/></br><p><i style='color:blue'>Shopname</i>: ${props.shopname}</br><i style='color:blue'>Prod</i>: ${props.name} &nbsp;<i class="fas fa-map-marker-alt"></i>${nFormatterdist(props.distance,1)}&nbsp; $${nFormatter(props.price,2)}</p>`;
          
           } else {
             content = `<img                                                       src=""             /></br><p><i style='color:blue'>Shopname</i>: ${props.shopname}</br><i style='color:blue'>Prod</i>: ${props.name} &nbsp;<i class="fas fa-map-marker-alt"></i>${nFormatterdist(props.distance,1)} &nbsp;  $${nFormatter(props.price,2)}</p>`;
...
function set_map(map){
  ...
  function get_bounds_and_resize_MapView(redraw){
    var layers_count =0
      // Create new empty bounds
      let bounds = new L.LatLngBounds();
      // Iterate the map's layers
...
function getPosition(position){
  ...
  if (get_transition_metre(5)){// at every 5 metres change redrw map
    ...
    if (t1 !== null){
        ...
        console.log('seconds from last update', dif_seconds)
        //update time        
        t1 = Date.now();
      
        if (dif_seconds > 30)// only go here id 30 seconds has passed
          myScript(var_fly_bounds)
...
function myScript(redraw=true){
  ...
  counter +=1
  console.log("inside myScript counter: ", counter)
  // keep last position
  ...
  if (search_val.length>0){            
    dataurl = slug_data_url.replace(/12345/, search_val.toString()).replace(/67/, user_loc_x).replace(/89/, user_loc_y); 
